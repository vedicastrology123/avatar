...
public class VargaDataCalculator{
	...
	private int[][] vargaDataArray;
	//private static AllPlanets allPlanets;
	//private static double ascendant;
	private VargaData vargaData;

	public VargaData calculateVargaData(AllPlanets allPlanets, double asc){
	...
	private int[][] calculateAllVargaPositions(AllPlanets allPlanets){
		
		String[] planetNames = { "Lagna",
				"Surya",
				"Chandra",
				"Kuja",
				"Budha",
		...
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[0][i] = getRasi(allPlanets.getPlanet(planetNames[i]).getLongitude());
			//System.out.print(" Rasi Positions: " + vargaDataArray[0][i] + "\n");
		}
		
		//Hora
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[1][i] =  getHora(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			

		//Drekkana
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[2][i] =  getDrekkana(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		
		//Chaturtamsa		
		for(int i = 0; i<allPlanets.getSize(); i++){
			vargaDataArray[3][i] = getChaturtamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}
		  
			/*
			 * //Panchamsa for(int i = 0; i<allPlanets.getSize(); i++){ vargaDataArray[4][i]
			 * = getPanchamsa(allPlanets.getPlanet(planetNames[i]).getLongitude()); }
			 */
		 			
		//Saptamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[4][i] =  getSaptamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}						
		//Ashtamsa
		/*
		 * for(int i = 0; i<allPlanets.getSize(); i++){ vargaDataArray[6][i] =
		 * getAshtamsa(allPlanets.getPlanet(planetNames[i]).getLongitude()); }
		 */			
		//Navamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[5][i] =  getNavamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Dasamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[6][i] =  getDasamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Dwadasamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[7][i] =  getDwadasamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Shodasamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[8][i] =  getShodasamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Vimshamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[9][i] =  getVimshamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Siddhamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[10][i] =  getSiddhamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Bhamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[11][i] =  getBhamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Trimshamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[12][i] =  getTrimshamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Khavedamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[13][i] =  getKhavedamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Akshvedasamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[14][i] =  getAkshvedamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}			
		//Shastiamsa
		for(int i = 0; i<allPlanets.getSize(); i++){	
			vargaDataArray[15][i] = getShastiamsa(allPlanets.getPlanet(planetNames[i]).getLongitude());
		}
...
//Navamsa
	 ...
	 */
	public int getNavamsa(double deg)
	{
